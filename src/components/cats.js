...
const Cats = React.createClass({
    ...
    handleSubmit: function(e) {
        ...
        xhr({
            ...
            this.setState({
                results: results.filter(obj => obj.name.toUpperCase().indexOf(this.state.s.toUpperCase()) === 0)
            })
        })
    },
    ...
    render: function() {
        function showResults(list) {
            return list.map(cat => h('div', [
                h('img.mt3', {src: 'http://placekitten.com/150/100'}),
                h('h1', cat.name),
                h('p', cat.from)
            ]))
        }
